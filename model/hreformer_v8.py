"""HreformerV8: V7 with bounded wrist/ankle coordinate refinement."""

import torch

from model.distal_joint_refinement import BoundedDistalJointHead
from model.hreformer_v7 import HreformerV7


class HreformerV8(HreformerV7):
  """Add a zero-initialized distal head without changing the V7 trunk."""

  def __init__(self, nInChannel: int, nOutChannel: int):
    super().__init__(nInChannel, nOutChannel)
    self.distal_head = BoundedDistalJointHead(
      channels=128,
      hidden_channels=64,
      n_joints=17,
      max_correction=0.5,
