...
class Wisppn(nn.Module):
  def __init__(self, nInChannel: int, nOutChannel: int):
    ...
    # Actual input channels after reshape: nTimestep(9) * nSubcarrier(114) = 1026
    actual_channels = nInChannel * 9
    self.channel_adapt = nn.Sequential(
      nn.Conv2d(actual_channels, 150, kernel_size=1, bias=False),
      nn.BatchNorm2d(150),
      nn.ReLU(inplace=True),
    )

    self.wisppn = ResNet(ResidualBlock, [2, 2, 2, 2])
