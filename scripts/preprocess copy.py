...
from libs import figutils, nputils, utils
import config
...
confList: list[config.Pre] = [
  # config.Pre()
  config.Pre(
    rawDir=r"/media/zxx/新加卷/CSI/FramesRaw/free-small",
    parsedDir=r"/media/zxx/新加卷/CSI/parsed/free-small",
  ),
  config.Pre(
...
class Preprocessor:
  ...
  def extractSyncedCsi(self, frameRate: int, csiN: int, resampleRule: str = "1ms"):
    ...
    resampledPhaseNp, _ = nputils.interpolateCsi(
      rawPhaseNp, csiTimestampNp, resampleRule
    )
